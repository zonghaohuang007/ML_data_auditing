...
def construct_datasets(dataset, data_path, normalize=True):
    ...
    if dataset == 'CIFAR100':
        ...
        # trainset = torchvision.datasets.ImageFolder(root=data_path + '/cifar100/train', transform=transforms.ToTensor())
        ...
        # trainset = torchvision.datasets.ImageFolder(root=data_path + '/cifar10/train', transform=transforms.ToTensor())
    ...
    if dataset == 'CIFAR100':
        ...
        # validset = torchvision.datasets.ImageFolder(root=data_path + '/cifar100/val', transform=transform_valid)
    elif dataset == 'CIFAR10':
        validset = CIFAR10(root=data_path, train=False, download=True, transform=transform_valid)
        # validset = torchvision.datasets.ImageFolder(root=data_path + '/cifar10/val', transform=transform_valid)
    elif dataset == 'MNIST':
...
class TinyImageNet(torch.utils.data.Dataset):
    ...
    def __init__(self, root, split='train', transform=None, target_transform=None):
        ...
        # self.label_texts = self.label_texts[::4]  # for 50 classes
        ...
        if self.split == 'train':
            ...
            with open(os.path.join(self.split_dir, self.VAL_ANNOTATION_FILE), 'r') as fp:
                for line in fp.readlines():
                    ...
                    if label_text in self.label_texts:
        ...
        # print(label_text_to_word)
        # self.classes = [label_text_to_word[label] for label in self.label_texts]
        self.classes = [label for label in self.label_texts]

        # word_class = []
        # word_to_text = {}
        # for i in self.classes:
        #     if i in label_text_to_word.keys():
        #         word_class.append(label_text_to_word[i])
        #         word_to_text[label_text_to_word[i]] = i
        # print(word_class)
        # print(word_to_text)

        self.image_paths = [file_path for file_path in self.image_paths if os.path.basename(file_path) in self.labels.keys()]
